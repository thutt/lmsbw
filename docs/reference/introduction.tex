...
\texttt{mtree} variable to the full path of the executable.
...
\item Recurse only when needed\label{intro:build-component-criteria}
  ...
  See \xref{samples:linux-kernel} for benchmarks numbers on wrapping
  the Linux kernel build process with \lmsbw.
...
\item \lmsbwcomponent{<component-name>}
  ...
  Each component loaded is assigned to an associative array that holds
  all the data relevant to building the software.
...
During the execution of the build process, \lmsbw runs three different
\makefile processes:
...
\item Component Trampoline \makefile
  ...
  \makefile macros (\texttt{CC}, \texttt{LD}, etc.) for the toolchain
  (\xref{chap:toolchain-usage}) selected to build the component.
