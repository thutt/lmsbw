...
The biggest stumbling block to implementing a non-recursive build
system using \make is the complete lack of scoping; symbol scoping is
accomplished by traditional recursive use of \make, and is all but
non-existent in a non-recursive implementation.  This creates a
maintenance issue because the developers have to be very careful to
end up with no macro, target or implicit rule collisions.
...
In short, if your \nullbuild takes considerably more time than the
\mtree utility takes to execute on all your source directories, then
you can benefit from \lmsbw.
...
\item Disk Space
  ...
  those options are to make a \emph{debug} build, or to perform some
...
\item Uniform Interface
  ...
  source directory containing a \makefile.  Declaration includes
  information about the directory and options for building it,
  including which other component are prerequisites.
...
\lmsbw is not a system which will actually build your software.
Rather, it is a wrapper for your own build system that, in most cases,
will improve the performance of your build system.  You're probably
...
process when no work needs to be done.

Let's examine the \lmsbw process:

\begin{description}
\item Define Build Process API
...
\item Declare Components

  \lmsbw always automatically generates rules that are used by \make
  to build your product, but to be able to generate these rules, you
  must declare every component that is in your project.
  ...
  build for itself, and, when requested, installing it into the
  \texttt{DESTDIR} directory.

\item Recurse only when needed

  \lmsbw only maintains the inter-component dependencies that are
  explicitly defined by virtue of the way that the set of components
  included in your project have been declared.  In order to increase
  speed and decrease memory use, \lmsbw specifically maintains no
  dependency information about any source file contained in any
...
Primarily \lmsbwconfigurationfield{load-configuration-function}
(\xref{aa:load-configuration-function}) is set to the name of a Gnu
\make function that is responsible for setting up the configuration
for each component that will be included in your build.
...
After loading the master configuration file, \lmsbw will invoke the
component configuration-loading function
...
proejct.  Generally there is a one-to-one correspondence of components
to component configuration files, which you provide, and the called
function will load each of these files using \gnumake
\texttt{\$(include)} directive.

The component configuration files are, in fact, \makefile snippets
which can have rules that will override the default behavior of \lmsbw
(\xref{chap:overriding}).  There are sevaral samples included with
\lmsbw that show how rule overriding can be done.
...
\item \lmsbwcomponent{<component-name>}
  ...
  hold all the data relevant to building the software.
  ...
  include information such as the source directory.  A
...
\item \lmsbw \makefile
  ...
  configuration and inter-dependencies between high-level components.
...
\item Component Trampoline \makefile
  ...
  standard \makefile macros such as \texttt{CC} and \texttt{LD} that
