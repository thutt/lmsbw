...
\item Uniform Interface
  ...
  simply execute the \texttt{report} verb (\xref{usinglmsbw:report}),
  but it can also be executed on a particular component with this
  syntax: \texttt{report.dailyprocessing}.
...
referring to \xref{samples:generate}.  To see the relationship between
\emph{number of component configuration files} and \emph{\lmsbw
  startup time}, please refer to \xref{samples:startup-time}.
...
in your project.  Generally there is a one-to-one correspondence of
