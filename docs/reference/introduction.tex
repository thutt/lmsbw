...
scoping; in the traditional use of \make, symbol scoping is
accomplished by recursive invocation, but is non-existent in a
non-recursive implementation.  The lack of scoping quickly becomes a
maintenance issue because the \makefile developers must be careful to
avoid macro, target and implicit rule collisions.
...
\item Extensible
  ...
  it's easy to add new features.  See \xref{chap:extending}.
...
referring to \xref{samples:generate}.  If you are concerned that a
...
\item Recurse only when needed\label{intro:build-component-criteria}
  ...
  Kernel.  See~\xref{samples:linux-kernel} for benchmark numbers on
...
\item \makefile Simplicity
  ...
  \item Component Trampoline \makefile
    ...
    toolchain (\xref{chap:toolchain-usage}) selected to build the
    component.
...
\lmsbwconfiguration (\xref{variables:lmsbw-configuration}) variable.

Primarily, you must declare a \gnumake function that will load your
component configuration
(\xref{variables:load-configuration-function}), and you must indicate
what \emph{kind} (\xref{variables:kind}) of components your project
will use (\xref{variables:component-build-support}).
...
(\xref{variables:load-configuration-function}).  This function is responsible
