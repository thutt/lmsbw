...
This is handy if the wrapped build process does not have a compliant
build process~(\xref{wrap:component-makefile}).  For example, if the
wrapped \makefile did not have an \emph{install} rule, it could be
overridden to manually install the needed files into \destdir.
