...
\section{Build Output Differentiation}
...
\begin{itemize}
\item{Different products are produced}
\item{Same master configuration file}
\item{Same set of components for each product}
\item{Component build output differs via conditional compilation}
\end{itemize}

In the case described above, the component's configuration is usually
controlled via environment variables that are proxied to the toolchain
and used in conditional compilation.  The problem is that this will
cause each SKU to share the same install directory.
