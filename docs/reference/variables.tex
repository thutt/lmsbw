...
\todo{Fix this label to be variables:}
...
\todo{Need an xref to the function which sets the value}
...
\apiref{api:build-root-directory}
...
\apiref{api:build-target}\label{variables:build-target}
...
component.\todo{xref overriding configuration rules}
...
(\xref{chap:overriding}), should install files into a concatenation of
...
\apiref{api:install-target}\label{variables:install-target}
...
support for new values of this key.\todo{Need a chapter on how to add
  new component types.}
...
\apiref{api:source-mtree-manifest}
